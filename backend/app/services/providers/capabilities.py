...
PROVIDER_CAPABILITY_MATRIX: dict[str, dict[str, dict[str, str]]] = {
    ...
    "opencode-cli": {
        "config": capability("read_only", "Configuration", "OpenCode JSON configuration is detected for Agent Mail setup, but not broadly editable in this build."),
        "sessions": capability("write_capable", "Agent Bridge Sessions", "Agent Bridge can discover and launch OpenCode sessions."),
        "spawn": capability("write_capable", "Spawn Sessions", "Agent Bridge can launch OpenCode sessions."),
        "resume": capability("write_capable", "Resume Sessions", "OpenCode continue and session flags are available."),
        "fork": capability("unsupported", "Fork Sessions", "OpenCode fork mode is not exposed through Deck yet."),
        "mcp": capability("write_capable", "MCP Servers", "Agent Mail can install an OpenCode MCP server entry."),
        "plugins": capability("write_capable", "Plugins", "Agent Mail can install a small OpenCode lifecycle plugin."),
        "permissions": capability("unsupported", "Permissions", "OpenCode permissions are launch flags and provider config, not a Deck-managed page yet."),
        "commands": capability("unsupported", "Commands", "OpenCode command configuration is not surfaced in this build."),
        "agents": capability("read_only", "Agents", "OpenCode agents can be selected at launch but not edited in this build."),
        "skills": capability("unsupported", "Skills", "OpenCode skills are not surfaced in this build."),
        "hooks": capability("write_capable", "Lifecycle Plugin", "Agent Mail uses an OpenCode plugin for lifecycle events and context injection."),
        "memory": capability("unsupported", "Memory", "OpenCode memory configuration is not surfaced in this build."),
        "output_styles": capability("unsupported", "Output Styles", "OpenCode output styles are not surfaced in this build."),
        "statusline": capability("unsupported", "Status Line", "OpenCode status line settings are not surfaced in this build."),
        "usage": capability("read_only", "Usage", "OpenCode stats are available through the CLI but not surfaced in this build."),
        "context": capability("unsupported", "Context", "OpenCode context diagnostics are not surfaced in this build."),
        "doctor": capability("unsupported", "Doctor", "OpenCode doctor diagnostics are not surfaced in this build."),
        "backup": capability("unsupported", "Backup", "OpenCode backup/export is not supported in this build."),
        "restore": capability("unsupported", "Restore", "OpenCode restore is not supported in this build."),
        "plans": capability("unsupported", "Plans", "OpenCode plan snapshots are not surfaced in this build."),
    },
}
