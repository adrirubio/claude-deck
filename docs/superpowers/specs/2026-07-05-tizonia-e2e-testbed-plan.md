...
- **T-S2 — auto-merge, checks green:** a trivial `agent-ready` issue, scope `merge_policy=auto`, branch protection off → brain auto-merges once, `auto_merged_at` set. *Hard gate.* (Choose the issue carefully — this lands real code in a public repo.)
...
## Exit criteria
- **T-Phase 0 + T-S1 + T-S2 + T-S3 + T-S5 must pass** — real CI baseline, code happy path, auto-merge, retry-recovery, design pipeline. These gate the Claude Deck integration-branch → master merge.
...
## Open questions specific to tizonia
1. **`design_label`:** the roadmap template doesn't create a `claude-deck-design`-equivalent. Either add one to tizonia, or configure the scope's `design_label` to an existing label (e.g. `area:docs` as a proxy?) — but note `area:docs` is a *routing* label, not a *pipeline* selector, so overloading it conflates two axes (exactly the C3 conflation the design spec warned about). Cleanest is to add a dedicated design label to tizonia. Decide before T-S5.
2. **Build cost:** tizonia's full build may exceed what an agent can do locally in a dispatch cycle, and may make CI slow enough to strain the poll/grace timing. Determine the minimal buildable target for both the agent's local check and the CI gate; this is the single biggest tizonia-specific risk to the plan working as written.
3. **Public-repo blast radius:** confirm you're comfortable with autonomous PRs (and, in T-S2, an auto-merge) on the public tizonia repo, or run the first passes against a private fork and only graduate to the canonical repo once trusted. A fork also sidesteps the "auto-merge into public revival" stakes for the initial gate.
