...
**Date:** 2026-08-05
**Status:** Design, revision 11 — revised after a tenth implementer review of `2503b9d` returned five blockers and three corrections. All five confirmed, and three are **broader than reported**: PR2's credential helper is a lease-token consumer that does not appear in `app/` at all, so it is a *new* consumer falsifying a PR1-era claim rather than one the analysis overlooked (§4.6a); the only reachable route back to `pending` does not merely fail to preserve a prepared attempt, it clears `approval_round_count` and `last_verified_sha` (§4.2b.1); and the backstop this spec twice named as the reader of stale PID evidence **never examines a `dispatched` item at all**, so revision 10's severity account was wrong in its own favour (§4.6b). The organising correction is that **revision 10 reasoned about PR1 from pre-PR0 `master`** — it measured the authority available today and wrote it into a PR that lands after PR0 supplies more. **Not yet approved for implementation planning.**
...
| `GithubWorkspaceResponse.lease_token` (`schemas.py:2245`) leaves the agent-reachable projection | **PR1** | The obligation created by PR1's authenticated claim path: once the token has a real delivery channel, a listing route that hands it to any caller is the hole PR1 closes elsewhere. **But not a one-line deletion** — operator force-release requires `expected_lease_token` (`schemas.py:2256`, `agent_teams.py:693`) and this route is the only way to learn it, so PR1 owns the choice between moving the field behind the operator surface and changing force-release's contract (§4.6a). PR1, because PR1 is the PR whose authorization is decorative without it |
| §4.2b.2's `POST .../resume-attempt` (operator only) | **PR1** | It is the only route out of `prepared_owner_unavailable`, which PR1 introduces. Measured: `dispatch_pending` selects `pending` by equality, so PR1 without this endpoint ships an escalation with **no** exit that preserves the attempt — the sole existing route to `pending` clears `approval_round_count` and `last_verified_sha` (§4.2b.1). An escalation whose recovery ships later is an escalation with no recovery |
...
#### 4.2b.2 `POST /agent-teams/{preset_id}/work-items/{item_id}/resume-attempt` — operator only

One endpoint serves two needs that arrive from different directions: §4.2b.1's `prepared_owner_unavailable` recovery, and §3.5a point 4a's genuine leader case (*the owner is wedged and a human wants the item moved*). Both are an operator saying "I have looked at this and the attempt should continue" — a claim no agent can make on its own behalf, which is why the leader row came off §3.5a's matrix.

| | |
|---|---|
| Caller | **operator only** — the same unauthenticated-but-human surface as force-release (`agent_teams.py:693`). No session token, no slot derivation: there is no agent identity to check because no agent may call it |
| Body | `resume: true`, and optionally `reassign_to_slot_id` for the wedged-owner case |
...
| Cause check | the reason for the escalation must be **gone**: the owner slot must resolve inside the scope's preset and be `enabled`. Still disabled ⇒ `409 owner_still_unavailable`, naming the slot. Without this the row bounces straight back to `escalated` on the next poll and the operator learns nothing |
| Liveness check | **required, and this is the check the agent route could not perform.** If the previous owner's pane is still alive, resuming hands a live agent's work to a second one. Reuse §4.6b's kernel-derived evidence on the leased workspace: if `_owner_process_is_alive` is true and the recorded pane is *not* the slot being resumed to ⇒ `409 previous_owner_still_alive`, with the pid and the pane in the detail. An operator who genuinely wants to override force-releases first — an action that already exists and already reads as dangerous |
| Writes | `dispatch_status = "pending"`, `escalation_reason = None`, `pending_reason = None`. Optionally `owner_slot_id = reassign_to_slot_id` and `routing_method = "operator_resume"` |
...
|---|---|---|---|
| `release_by_token` (`:364`) | shipped | `403` before the token is read | the owner check at `:334` runs first (now on the derived slot, §3.5a) |
| `touch_owner_contact` (`:376`) | shipped | never called | gated on the reporter being the owner (`:371-373`) |
...
- It is not nullipotent. See below: it stamps the caller's pane identity onto the lease.
- **It is the write that repairs blocker 4.** `accept_handoff` cannot stamp the target's pane pid because it has no way to know it — the accepting call may arrive from anywhere. The *continuation claim* does know: PR0's dependency has just derived the caller's bound pane from the kernel. So the claim is where the liveness evidence transfers, and making the endpoint a read was what left revision 10 with no place to put that write. §4.6b states the transfer.
...
**Grace mode does not grant a bearer secret.** §3.4a lets PR0 deploy with `mail_capability_tokens_required = False` so nothing breaks on rollout, and a tokenless caller then falls back to today's behaviour. That fallback is acceptable for reports, which only *reduce* Deck's confidence (§3.5a point 5). It is **not** acceptable here, because this endpoint's response body contains a live lease token: a tokenless caller in grace mode receives `409 tokens_not_enforced` naming the setting to enable, never the context. Continuation context is the one route that is strict while the rest of the surface is permissive, and the reason is the payload, not the caller.
...
1. **Withdrawn, and it leaves an obligation behind.** Revision 10 accepted disclosing a lease token to a caller whose slot identity is self-asserted, on the ground that `GET /github-scopes/{id}/workspaces` (`:569`) already leaks the token with no auth at all. The premise is true — `GithubWorkspaceResponse.lease_token` (`schemas.py:2245`), filled unconditionally by `_workspace_response` (`agent_teams.py:185`) — and the inference was wrong twice over: an existing hole is not a licence to add a second one, and PR0 removes the need for either. Post-PR0 the caller's slot is kernel-derived, so the token goes only to the pane bound to the owning slot.
  ...
  So the `:569` leak becomes an obligation rather than a precedent — **but not the one-line deletion it looks like, and this is measured rather than assumed.** No frontend code reads the field (grep across `frontend/src/`: zero hits for `lease_token`/`leaseToken`; there is no workspaces UI yet). The blocker is on the backend: operator force-release requires `expected_lease_token` in its request body (`schemas.py:2256`, compared at `agent_teams.py:693`), and this listing route is the **only** way an operator can learn that value. Deleting the field would remove the sole exit path from a stuck lease — the exit path §4.2b.1 leans on twice — to close a hole PR0 has already made unnecessary to close this way. The requirement is therefore: **`lease_token` leaves the agent-reachable projection and stays reachable to the operator**, either by moving it behind the same operator-only surface as force-release, or by having force-release stop requiring a value the caller must first read (an explicit `force: true` plus the lease's `leased_at`, which is not a secret, is the simpler shape). Whichever is chosen, PR1 owns the choice, because PR1 is the PR whose authorization is decorative without it.
...
**The requirement.** The evidence transfers with the authority, at the moment the target proves it is a real pane:

1. `accept_handoff` **clears** `leased_owner_pid`, `leased_owner_proc_start`, and `lease_last_owner_contact_at`. It does not leave A recorded as B, and it does not guess B. Cleared is the honest state: `_owner_process_is_alive` returns `True` when `leased_owner_pid is None` (`:85-86`), so a cleared row is treated as "cannot tell, do not reclaim" — fail-safe, and the same answer it already gives for a workspace leased before the pid columns existed.
2. The continuation claim (§4.6a) stamps `leased_owner_pid`, `leased_owner_proc_start` from PR0's kernel-derived pane binding, and `lease_last_owner_contact_at` from the clock, in one transaction with the read it returns. This is why the endpoint is a `POST`.
3. Between (1) and (2) the lease is held with no liveness evidence and no contact stamp. That window is bounded by the brief telling the target to call `deck_get_work_item_context` first, and it fails safe by (1)'s `None` semantics rather than by timing.
...
11. `accept_handoff` clears all five ack fields and **keeps** the nonce; the previous owner's approval does not carry to the new owner, *and* the new owner can still be acked (proving the §4.2 deadlock is avoided).
    ...
    Three parts. (i) **The defect**, unchanged from revision 10 and still worth pinning: `leased_owner_pid` and `leased_owner_proc_start` are untouched by the real `accept_handoff`, and `inspect.getsource(accept_handoff)` mentions neither them nor `workspace` at all. (ii) **The refutation of the claimed repair** — with A's pane *alive* and recorded on the row, `reclaim_stale` releases `0` whether `lease_last_owner_contact_at` is NULL or freshly stamped:
    ...
    Assert both, in that order, in one test. The identical outcome is the finding: `_owner_process_is_alive` is evaluated **before** the contact branch (`:296` before `:299`), so the stamp is read by a branch this case has already short-circuited past. Delivery fixes *who may act*; it does not fix *whose liveness is observed*. And (iii) **the repair**, which needs no new column: with the row corrected to B's live pane, `_owner_process_is_alive` flips `False → True` and `reclaim_stale` releases `0` for the right reason:
    ...
    Finally the ordering assertion §4.6b's requirement rests on: after `accept_handoff`, all three of `leased_owner_pid`, `leased_owner_proc_start` and `lease_last_owner_contact_at` are `NULL`, and `_owner_process_is_alive` on that row returns `True` (`:85-86`) — cleared is fail-safe, so the window between the handoff and B's continuation claim protects B rather than exposing it. Assert the `True`, not just the NULLs: the NULLs are the mechanism and the `True` is the property, and an implementer who clears the columns while "fixing" the predicate to return `False` on a NULL pid would satisfy the mechanism and invert the property.
    ...
    - slot **B**, a sibling of the same preset on the same repo, with a valid session token bound to B's own pane, calls `claim-continuation` ⇒ `403 slot_claim_mismatch`. This is the whole test: pre-PR0 this call is indistinguishable from A's.
    - slot B with a valid token **and** `reporting_slot_id: A` in the body ⇒ still `403`, because the derived slot governs and the body's claim is at most corroboration (§4.6a). Assert no payload and, specifically, that the response body contains no `lease_token` substring — a route that computes the payload and then refuses would leak on the error path.
...
| `accept_handoff` leaves `leased_owner_pid` set instead of clearing it | **37r-4** — an alive ex-owner then pins the lease through `reclaim_stale`'s PID branch regardless of contact stamps, which is the ordered-conjunction finding |
...
| `accept_handoff` re-stamps `leased_owner_pid` from the accepting **request** | **37r-4** — the accepting call has no access to the new pane's pid (the target reports over HTTP, not from the pane), so this writes whatever process happened to make the call. The fix is to **clear** the columns at `accept_handoff` and stamp them from PR0's kernel binding at the continuation claim — two writes in two places, not one write in the wrong place (§4.6b) |
| the handoff clears the pid but `_owner_process_is_alive` is "fixed" to return `False` on a NULL pid | **37r-4**'s last assertion — the clear is only fail-safe *because* the predicate returns `True` on NULL (`:85-86`), so this mutation inverts the property while satisfying the mechanism, and the window between handoff and claim starts reclaiming live work |
...
31. **The continuation claim derives its caller from PR0's kernel binding, and `reporting_slot_id` is never an authority input.** Revision 10 wrote this criterion as an honesty statement — the endpoint is "owner-checked, matching `agent_teams.py:334`," with the spec admitting plainly that the check is not authentication. Both halves were wrong in the same way, and the way is blocker 1: **the measurement was taken on pre-PR0 `master` and written into a PR that lands after PR0**. There is no capability token on the dispatch surface *today*; PR0 adds one, and PR1 is where this endpoint ships. A design that declines authority its own earlier PR has already supplied is not being modest, it is being stale.
    ...
    Two consequences from revision 10 are withdrawn as unnecessary rather than accepted (§4.6a), one is retained and narrowed, and the `:569` `lease_token` projection becomes an obligation with a measured constraint: it must leave the agent-reachable projection **and** stay reachable to the operator, because operator force-release requires `expected_lease_token` (`schemas.py:2256`, compared at `agent_teams.py:693`) and that listing route is the only way to learn the value. The endpoint is a **`POST .../claim-continuation`**, not a `GET`, for three reasons: the response holds a live bearer secret, the call is not nullipotent, and it is the write that repairs blocker 4 by stamping the new owner's pane identity (§4.6b). And because the old check read a column the caller population can write, `initiate_handoff` ships its **missing owner check** — on the *derived* slot, refusing `403 not_item_owner` per §3.5a point 4a — plus application-level target validation, in the **same** PR: measured, a non-owner from another preset takes ownership in two calls, and the only existing validation on the target is a **pragma-dependent** FK surfacing as an uncaught `IntegrityError`, a 500 (§4.6a, §2.1, §4.8 tests 37r-2, 37r-2a, 37r-3, 37r-5, 37r-7).
