...
**Date:** 2026-08-05
**Status:** Design, revision 10 — revised after a ninth implementer review of `5c33c72` returned three blockers and seven corrections. All three blockers confirmed; two of the corrections **refuted the spec's own text** rather than merely amending it, and one of those refuted the first replacement rule this revision drafted for it (§5.8, 29-a2). The largest finding is that a handoff **splits the authority to finish the item** across two processes so that neither can — a liveness defect in shipped code, not a design gap, and the reason the answer to "rotate the lease token?" is *deliver it instead* (§4.6a). **Not yet approved for implementation planning.**
...
| Consumer | Ex-owner holding the live token | Why |
|---|---|---|
| `release_by_token` (`:364`) | `409` before the token is read | the owner check at `:334` runs first |
| `touch_owner_contact` (`:376`) | never called | gated on `report.reporting_slot_id == item.owner_slot_id` (`:371-373`) |
| operator force-release (`:693`) | not an agent path | compares against the current token deliberately |

Both agent-facing consumers are behind an owner check the ex-owner fails, so rotation would add no protection. What it *would* do is invalidate the only copy of a capability that has no delivery channel to the new owner — making B's position strictly worse. **Decision: do not rotate the lease token on handoff. Deliver it.** Measured confirmation that delivery is sufficient: B, holding the actual live token, releases successfully and the lease clears (`leased_item_id=None`, `token_present=False`) — the token is a bearer capability on the *acquisition*, not bound to the slot that received it.
...
`GET /api/v1/agent-teams/github-work-items/{id}/continuation-context?reporting_slot_id=<n>` returns the server's own view of what the caller owns: `work_item_id`, `issue_number`, `issue_title`, `issue_url`, `issue_type`, `repo_owner`/`repo_name`, `dispatch_status`, `approval_round_count`, `dispatch_nonce`, `dispatch_head_ref`, `workspace_path`, `lease_token`, `leader_member_id`, and `status_note`. Every field is read from the row — nothing is recomputed, so the head the target is told is by construction the head `pr_ready` will check it against (§4.1's rung).

`deck_get_work_item_context(work_item_id)` in the shim, filling `reporting_slot_id` from the registered member exactly as `deck_report_dispatch_status` does (`:626`) rather than from a tool argument.

**Authorization, stated honestly rather than assumed.** The check is `reporting_slot_id == item.owner_slot_id`, matching `workspace_released` (`:334`) — the only existing branch that checks ownership at all. It is not a capability-token check, because **there is no capability token on either agent-facing router**: zero `Header()` parameters on any route in `agent_teams.py` or `agent_mail.py`. PR0's mail tokens (§3) gate the *mail* surface; the dispatch surface has none, and bringing it under tokens is out of scope for PR1 (§6).

What that check is actually worth, measured:
...
Three consequences follow, and the design accepts all three explicitly:

1. The endpoint discloses a lease token to a caller whose slot identity is self-asserted. That is **not a new exposure**: `deck_report_dispatch_status` already accepts `workspace_released` on the same self-asserted field, and the same caller could already read the token from `GET /github-scopes/{id}/workspaces` (`:569`), which has no auth at all. The endpoint is bounded by the *same* check as the strongest existing one, and no weaker.
2. Because the check is only as good as the registration, it must be paired with the write-side fix below rather than presented as authorization on its own.
3. The threat model this closes is **confusion, not intrusion**: a team-mate agent that does not know what it owns. It does not defend against a hostile process inside the repo, and §6 says so.
...
**One more consequence of the handoff, found while measuring this and fixed by the same delivery.** `accept_handoff` touches no workspace column, so the lease's liveness evidence still describes the previous owner's process:
...
`_owner_process_is_alive` (`github_workspace_service.py:83-95`) is answering a question about the wrong process from the moment the handoff lands. Once A's pane exits, the backstop reads "owner dead" about an item B is actively working — and B cannot correct it, because `touch_owner_contact` no-ops without the token (`:264-270`), measured: B reports `triaging`, `status_note` is written, `lease_last_owner_contact_at` stays `NULL`. This is [[invariant-evidence-freshness]] again: the safety rule is sound and the evidence it reads is stale. `reclaim_stale`'s quiescence guards (`:298-323`) mean this is not immediately destructive, which is why it is a defect and not an incident. Delivering the token fixes it without a new column: with the token, B's ordinary status reports stamp contact, and the evidence tracks the agent actually working.
...
11. `accept_handoff` clears all five ack fields and **keeps** the nonce; the previous owner's approval does not carry to the new owner, *and* the new owner can still be acked (proving the §4.2 deadlock is avoided).
    ...
    37r-4. **After a handoff, the lease's liveness evidence names the wrong process — and delivery repairs it.** Assert `leased_owner_pid` and `leased_owner_proc_start` are unchanged by the real `accept_handoff`, and that `inspect.getsource(accept_handoff)` mentions neither `leased_owner_pid` nor `workspace` at all. Then the consequence, in two steps: B reports `triaging` **without** a token ⇒ `status_note` is written but `lease_last_owner_contact_at` stays NULL (`touch_owner_contact`'s deliberate early return at `:264-270`); B reports `triaging` **with** the token from `deck_get_work_item_context` ⇒ the stamp lands. The second half is what makes this a test of §4.6a rather than a bug report: the fix is delivery, not a new column. Include the negative control that the mechanism itself works — the original owner with its briefed token stamps contact — so the first half cannot be misread as a broken code path.

    37r-5. **The context endpoint's authorization is the owner check, and its limit is stated.** Two assertions. (i) A non-owner slot calling the endpoint ⇒ `409`, no payload. (ii) The **honesty** assertion, which is the one worth writing: register two slots of the same preset, same provider, same repo, with the second process claiming the first's `team_slot_id` via the real `register_session`, and assert the resulting `member.team_slot_id` is the **claimed** slot and `display_name` the claimed slot's name. Then the negative control — the same claim with a `cwd` on a different repo resolves to `team_slot_id=None` and `participant_kind='repo'`. Measured both ways. This test does not assert a defence; it pins the *scope* of one, so that a later reader cannot mistake `reporting_slot_id` for an authenticated identity. If someone strengthens registration, (ii) fails and the spec paragraph it guards must be rewritten — which is the correct outcome.
...
| the context endpoint omits `lease_token` from its payload ("a read should not hand out capabilities") | **37r-1** and **37r-4** — B can then never release the workspace nor stamp contact evidence, which is the whole defect 37r measures |
...
| the context endpoint is unauthorized, or checks `handoff_target_slot_id` instead of `owner_slot_id` | **37r-5** — the target column is NULL after `accept_handoff` (`:704`), so a target-based check refuses the very caller it exists for |
...
| `accept_handoff` re-stamps `leased_owner_pid` to "fix" the liveness evidence | **37r-4** — the accepting call has no access to the new pane's pid (the target reports over HTTP, not from the pane), so this writes the *reporter's* pid; delivery is the fix, and the test asserts the stamp lands via `touch_owner_contact` |
...
29. **A handoff target can finish the item it now owns, because the handoff transfers authority and not just identity.** Today it cannot, and the defect is in shipped code: `owner_slot_id` moves to the target while the lease token — delivered *only* in the launch brief, interpolated four times — stays with the original owner, and the release route requires both. Measured through the real route: the ex-owner is refused `409 only the owner slot may release its workspace`, the new owner is refused `400 lease_token required`, a guess is refused `409 lease_token does not match`, and the workspace stays leased through all three. The target's own reports cannot even record liveness, because `touch_owner_contact` returns early without the token — so `leased_owner_pid`, which `accept_handoff` never re-stamps, keeps naming the previous owner's process while the backstop reads it. The criterion is met when the target obtains its work item's nonce, exact stored head, workspace path, approval round, and **live lease token** through an authenticated agent-facing read (`deck_get_work_item_context`, §4.6a) and releases with them — asserted through the real tool, never by injecting fixture knowledge, with `deck_list_work_items`'s exact five keys as the negative control (§4.8 tests 11g, 37r, 37r-1, 37r-4).

30. **The lease token is delivered on handoff, not rotated.** The ninth review offered two options — rotate, or prove the retained token useless. Measured, there is a third and it is the correct one: the retained token is *already* useless to the ex-owner, because both agent-facing consumers (`release_by_token` via the route, and `touch_owner_contact`) sit behind an owner check the ex-owner fails; the token was never invalidated, and it grants nothing. So rotation would protect against nothing while destroying the only copy of a capability that has no delivery channel to the new owner — leaving the target exactly as stuck as it is today. Delivery by server-authored mail is also **not a new class of mechanism**: `remind_held_leases` already interpolates the live token into a message whose recipient is resolved from the *current* owner. It cannot simply be reused, because it is gated on `_RELEASABLE_STATUSES` and `'dispatched'` is not in it — measured **0** reminders and **0** messages throughout the target's entire working life — and widening that gate would fire release reminders at agents mid-work, contradicting the function's own purpose (§4.6a, §4.8 tests 37r-1, 37r-6).

31. **The continuation-context endpoint states the authority it has, and does not borrow authority it lacks.** It is owner-checked, matching `agent_teams.py:334`, and the spec says plainly what that check is worth rather than calling it authentication: there is **no** capability-token parameter on either agent-facing router, and `reporting_slot_id` is server-*stored*, not server-*verified* — `_slot_matches_registration` compares only `provider` and `repo_id`, identical for every slot on one repo, so a caller's own `CLAUDE_DECK_TEAM_SLOT_ID` decides which slot it claims to be (measured: one slot's process registers as another and is named as that other slot; control: a different `cwd` is refused down to `participant_kind='repo'`). Real discriminating power at repo granularity, none at slot granularity — which is the granularity every owner check in the dispatch flow is written at. Three consequences are named and accepted rather than papered over. And because the check reads a column the same caller population can write, `initiate_handoff` ships its **missing owner check** (`reporting_slot_id == item.owner_slot_id` ⇒ else `409`) and application-level target validation in the **same** PR: measured, a non-owner from another preset takes ownership in two calls, and the only existing validation on the target is a **pragma-dependent** FK surfacing as an uncaught `IntegrityError` — a 500 (§4.6a, §2.1, §4.8 tests 37r-2, 37r-3, 37r-5).
