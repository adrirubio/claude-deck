...
20. **`require_operator` refuses every credential an agent can obtain.** For **each** of force-release and the workspace listing, six callers:
    ...
    | the configured operator token | `200`/`204` — the positive control, without which a dependency that refuses everything passes all five rows above |
...
The last row is deliberate. A timing-safe comparison is not test-observable, so listing it as a review item is honest; claiming a test covers it would not be. Note the distinction from test 20's prefix and trailing-byte rows: those catch a comparison that is *wrong*, which is observable; only a comparison that is right-but-not-constant-time is invisible, and that is the residue the review item covers.
...
- `backend/app/config.py` — one setting for the operator token. Defaults matter here: an empty default must refuse **every** request rather than admit every request, because the shipped posture of an unconfigured install is the one nobody chose deliberately. That is the same fail-closed direction as `bind_unverifiable`.
...
- **Operator action required at deploy:** set the operator token, restart agent panes, then flip `mail_capability_tokens_required` to `True` (§3.4). Live: 150 `mcp` session rows, 7 currently connected. The token must go somewhere the launch path does not pass into panes (§3.6a's deployment note).
...
**Deployment ordering, therefore, is part of PR1 and not a runbook footnote:** deploy PR0 → restart panes so every agent registers and mints a session token → set `mail_capability_tokens_required = True` → deploy PR1. Autonomy stays off throughout (§6), so nothing dispatches into the window. PR0 alone remains behaviour-preserving, which is its entire design goal; **PR1 is the point at which identity becomes a hard runtime prerequisite**, and saying so is more honest than shipping a matrix that silently degrades to today's behaviour whenever a flag is false.
...
11. `accept_handoff` clears all five ack fields and **keeps** the nonce; the previous owner's approval does not carry to the new owner, *and* the new owner can still be acked (proving the §4.2 deadlock is avoided).
    ...
    Finally the assertion §4.6b's requirement rests on, and it is an assertion about the **post-PR1 implementation**, not about `master`. Revision 11 ended this test by requiring that `accept_handoff` leave all three columns `NULL` — which contradicts part (i) of the same test, where the *unmodified* function is pinned as never touching them, and contradicts §4.6b, which now requires it to **set** them. Two assertions about one function, one describing the defect and one describing a fix that no longer is the fix. Both cannot pass against a single implementation, and an implementer would have had no way to satisfy the file. Replaced with the behaviour the section now specifies:
