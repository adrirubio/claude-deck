...
```
  ...
  2. **The lease is cleared by one conditional mutation whose predicate is the acquisition that was inspected**, issued after the awaited inspection, not before it: `UPDATE ... WHERE id = <workspace_id> AND scope_id = <scope_id> AND leased_item_id = <captured> AND leased_at = <expected_leased_at>`. The stored `lease_token` may be added as a further discriminator, captured server-side and **never** returned to the operator — which is the useful role for the token the review identified, and it costs nothing because the value is already in hand.
  3. **Exactly one affected row, or nothing happened.** Zero rows ⇒ `409 lease_changed`, the current lease untouched, and **no success log** — the existing `logger.warning` fires *before* the release today (`agent_teams.py:701-710`), so a naive port would record a force-release that did not occur. Exactly one is a guarantee rather than a hope: `leased_item_id` is `UNIQUE` (`database.py:319`, `uix_workspace_leased_item`).
...
```
  ...
  The replacement dies. **The window is a DB round trip rather than two `git` subprocesses**, which is why this is a §7 deferral and not a fifth requirement: the honest statement is that its guarantee holds by timing rather than by construction, and a same-transaction conditional `UPDATE` would make it hold by construction. Recorded because the docstring is an absolute claim and someone will one day cite it as the reason a report cannot misfire — and because it shows the defect class is structural to this service rather than particular to one route. PR0 does not touch it: `release_by_token` is on the agent's report path, its caller sends a token it holds for a lease it holds, and widening PR0 to a second concurrency fix trades a bounded review for an unbounded one.
...
- **Making `release_by_token`'s guarantee structural.** Measured in §4.6a: it compares the acquisition token in one statement and clears the row in another, so a replacement acquisition landing between them is destroyed — the same defect class as force-release, in the helper whose docstring says it prevents exactly that (`github_workspace_service.py:181-185`). The window is a DB round trip, not two `git` subprocesses, and the caller is an agent reporting on a lease it holds rather than an operator destroying someone else's, so the exposure is far smaller. The fix is the same shape as §4.6a's requirement 2 — one conditional `UPDATE` keyed on the workspace row and the token — and it is deferred so that PR0 carries one concurrency change rather than two. **What should not survive this deferral is the docstring's absolute phrasing**; whoever next edits that function should either make the claim true or soften it, because a comment asserting a guarantee the code does not provide is the failure mode this spec keeps finding one layer up.
...
37. **A force-release destroys only the acquisition the operator inspected.** The confirmation value and the destructive write are one conditional mutation, not a comparison followed by a helper call. Measured on `master`: the lease check sits above two awaited `git` subprocesses in `pending_work`, and `release(db, item_id)` selects `WHERE leased_item_id == item_id` with **no workspace id and no scope id** (`github_workspace_service.py:148-152`), so a request that inspected workspace X clears a replacement lease — including one held by a **different workspace Y**, which is broader than the twelfth review's same-item framing.
    Met when `force` is `Literal[True]` (so an ignored field is a validation error rather than a silent no-op), when the write is one `UPDATE` predicated on workspace id + scope id + captured `leased_item_id` + `expected_leased_at` issued **after** the inspection, when zero affected rows ⇒ `409 lease_changed` with the lease untouched and **no success log**, and when the interleaving and cross-workspace cases are both tested against rows read back rather than ORM objects (§4.6a, §3.7 test 22). The "exactly one row" assertion rests on `leased_item_id` being `UNIQUE` (`database.py:319`) — verified rather than assumed, and it refuted my own prediction that two matching rows could wedge the route permanently.
