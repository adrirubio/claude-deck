...
**Date:** 2026-08-05
**Status:** Design, revision 19 — **approved for implementation planning.**

**What revision 19 changed** — revision 18 wrote the actor capability as three route requirements and left the rest to the plan. Writing the plan meant implementing them, and the shortest implementation that satisfies revision 18's own words **is wrong in three measured ways**. All three are stated normatively here, because §3.6b's own rule is that the spec must name new semantics before the plan does, and because each one is the version an implementer reaches for first.

**(1) The ack cannot build its response through `request_status`, and the failure lands after the write commits.** `acknowledge_external_request` ends `return await self.request_status(db, actor, message_id)` (`:346`), and `request_status` calls `self.thread` (`:288`) → `_require_actor_owns_thread` (`:396-402`). So relaxing the ack's *own* ownership check (`:339-340`) is not enough: on an agent-created root the response path raises `PermissionError` while assembling the reply to a request that has already been committed. The route catches `ValueError` only (`external_agent_mail.py:228`), so `PermissionError` escapes. Measured through the real ASGI app:
...
A `500` after a successful write is the worst available outcome: the UI reports failure, the operator retries, and the state has already moved. §3.6b now requires the actor ack to project its response without passing through the ownership gate.

**(2) The actor ack is a silent no-op on one of the two ack buttons the UI actually has.** `ThreadDialog` acks a **pending** `handoff` (`:126-128`) and an **answered** `context_request` (`:129-132`). The member ack moves a pending handoff to `acknowledged` (`agent_mail_service.py:1298-1305`); `acknowledge_external_request` moves only `answered → acknowledged` (`:343-345`). Measured, an actor ack modelled on the external one returns `200` and changes nothing:

```
handoff status before: 'pending'
relaxed actor ack -> returned status 'pending'
handoff request_status after: 'pending'  <-- moved? False
member ack moves it: 'acknowledged'
```

Green tests, `200` responses, and a button that does nothing. The requirement now names both transitions.

**(3) The two ack buttons post two different message ids, and only one is a request.** The handoff button posts the **root** id; the answer button posts the **answer's** id (`:223`). An actor ack is root-scoped, and `kind not in {"context_request","handoff"}` refuses an `answer` — measured `400 "Message is not a request"`. So the frontend change is not "swap the helper": the answer-ack call site must send the root's id. Revision 18 said "update the three UI writes" without noticing that one of them changes its *argument*.

Two smaller findings ride along. **The reply relaxation admits a third row shape that exists in quantity:** the predicate "no *other* actor owns the root" also admits roots with both senders NULL, and the live database holds **105** of them — all `broadcast` or `message`, none a request (read-only query). Measured, a reply into an anonymous `message` root notifies its recipient, and into a `broadcast` root notifies nobody (`stored_no_recipients`) — harmless, and now stated so an implementer does not narrow the predicate to `sender_member_id IS NOT NULL` and break the 59 direct-message threads. **And `ThreadDialog`'s reply form requires a member sender** (`disabled={!sender}`, `:263`), which under §3.5's gate is exactly the write a browser can no longer make; §3.6b now states that the "Reply as" select is removed rather than left to produce `401`s.
...
| **reply** | relax `reply_in_thread`'s refusal so an actor may reply in a thread **no other actor owns**, persisting `sender_actor_id` with `sender_member_id = NULL`, and routed with `recipient_member_id=None`. An actor may still not reply in another *actor's* thread | `:258-259` refuses every thread the actor did not create, which is every thread that matters here |
| **ack** | a new actor-scoped acknowledgement that (a) refuses only when **another** actor owns the root, (b) moves a **pending `handoff`** and an **answered `context_request`** to `acknowledged`, (c) writes **no** member `MailReceipt.read_at`, no `acked_at`, no `last_seen_at`, no `mailbox_status`, and (d) projects its response **without** routing through `self.thread`'s ownership gate | `acknowledge_external_request` (`:330-346`) refuses on the same ownership check, moves only `answered`, and returns through the gate; the *member* ack (`agent_mail_service.py:1293-1295`) writes exactly the fields that must not move |
...
**The ack's response path is part of its contract, not an implementation detail.** This is requirement (d), and it exists because the obvious implementation — copy `acknowledge_external_request`, relax its one comparison — fails *after committing*. `acknowledge_external_request` returns `await self.request_status(...)` (`:346`); `request_status` calls `self.thread` (`:288`); `thread` calls `_require_actor_owns_thread` (`:396-402`). On an agent-created root that gate refuses, and the ack route catches only `ValueError` (`external_agent_mail.py:228`), so a `PermissionError` from the *response* path escapes the handler. Measured through the real app: `HTTP 500`, with `request_status` already `'acknowledged'` in the database. The actor ack must therefore build `ExternalAgentMailRequestStatus` from an ungated read — `agent_mail_service.get_thread`, which takes no actor and writes nothing (verified: it only projects through `_message_response`). Relaxing `_require_actor_owns_thread` itself is the alternative and is rejected: read scope is a separate decision from write scope, and the member thread route is already ungated (`agent_mail.py:78-87`, no dependency but `get_db`), so the UI's read path needs nothing.

**Both ack transitions are normative, because the UI has two ack buttons and the external ack serves neither.** `ThreadDialog` acks a **pending `handoff`** at the root (`:126-128`) and an **answered `context_request`** (`:129-132`). `acknowledge_external_request` moves only `answered → acknowledged` (`:343-345`), so on the handoff it returns `200` and changes nothing — measured `'pending'` before and after, where the member ack yields `'acknowledged'`. A specification that named only "records the acknowledgement" is satisfied by that no-op. The requirement is explicit: pending `handoff` → `acknowledged`, and answered `context_request` → `acknowledged`.

**The answer-ack call site must change its argument, not just its helper.** The handoff button posts the root's id; the answer button posts the **answer's** id (`:223`, `handleAck(answerToAck.id, ...)`), because the member ack is receipt-scoped and there is a receipt on the answer. An actor ack is root-scoped and refuses a non-request kind — measured `400 "Message is not a request"` for the answer's id, `200` for the root's. So the frontend change for that button is `root.id`. Relatedly, `answerAckMember` is gated on `root.sender_member_id` being truthy (`:132`), which is NULL on an actor-created request — so that button never renders for a thread the operator started, and after this change the ack it performs no longer needs a member at all.

**The reply relaxation's predicate is "no other actor owns the root", and that wording is deliberate.** It admits agent-created roots (`sender_member_id` set) and also roots with **both** senders NULL — today's anonymous operator compose. Those are not hypothetical: the live database holds **105** such roots, 59 `message` and 46 `broadcast` (read-only query, and zero actor rows exist yet). Measured, an actor reply into an anonymous `message` root notifies its recipient; into a `broadcast` root it notifies nobody and reports `stored_no_recipients`. Both are acceptable. The narrower predicate an implementer might reach for — `sender_member_id IS NOT NULL` — would refuse the operator's own 59 existing direct-message threads, which is a regression in the one surface this section exists to make usable.
...
**Two behavioural deltas come with moving the UI onto the external routes, and both are acceptable.** First, the external send path is **rate-limited** — 30 messages per 60 seconds per actor (`EXTERNAL_RATE_LIMIT_MAX_MESSAGES`, `:32-33`), refusing `429` with a `Retry-After` header — where the member route has no limit. Per-tab actor keys give each tab its own window, so an operator hits it only by sending 30 messages in a minute from one tab. The UI must surface the `429` rather than reporting a generic failure. Second, the external path calls `wake_members_with_results` and passes `auto_nudge=False` (`:170-177`), while the member route takes the default `auto_nudge=True` (`agent_mail_service.py:843`). The difference is the nudge **cooldown**: `auto_nudge_members` skips a member nudged within `AUTO_NUDGE_COOLDOWN_SECONDS` (`:1203-1211`), whereas the external path wakes unconditionally and returns per-recipient results. So the operator's mail wakes recipients *more* reliably and reports what happened, which is the better behaviour of the two; it is recorded here so a reviewer sees a consequence rather than an accident.

**The "Reply as" select is removed, and that is the visible consequence of this design.** `ThreadDialog`'s reply form requires an operator-chosen member sender — `disabled={!sender || !replyBody.trim()}` (`:263`) — and builds `sender_member_id: senderId` (`:159`). That is precisely the write §3.5 gates, so under enforcement it becomes a `401` from a browser that holds no session token. The control cannot be left in place: a select whose every submission fails is worse than no select. It is replaced by nothing — the operator's reply is actor-authored, attributed by `sender_type` in the block `MessageBlock` already renders. The `kind` computation at `:152-156` goes with it, since an actor reply is always `kind="message"`; an actor `answer`, if PR0 admits one, is a separate route and not something the reply box infers from a dropdown. Members are still listed in the dialog for the recipient picker in compose; only the reply-as selector goes.
...
- **6j. The ack's status code, on an agent-created root.** Assert **`200`**, not "not a 4xx". The obvious implementation — relax `:339-340` and keep the `return await self.request_status(...)` tail — commits the transition and *then* raises `PermissionError` from the response path, which the route does not catch: measured `HTTP 500` with `request_status` already `'acknowledged'`. A test asserting only the database state passes against it, and so does a test asserting `response.status_code != 400`. The code is the assertion.
- **6k. The ack moves a *pending* handoff.** Ack an agent-created `handoff` whose `request_status` is `'pending'` — the state `ThreadDialog`'s first ack button acts on (`:126-128`) — and assert it reaches `'acknowledged'`. An ack copied from `acknowledge_external_request` moves only `answered` (`:343-345`) and returns `200` having changed nothing: measured `'pending'` before and after. This test is what makes 6d's positive half honest, because 6d's own root is a *context_request* and passes without it.
- **6l. The answer-ack call site sends the root's id.** The actor ack refuses a non-request kind, so posting the *answer's* id — what `ThreadDialog:223` sends today — is `400 "Message is not a request"` (measured), while the root's id is `200`. Assert the refusal too: it is the only thing that distinguishes a frontend that was updated from one that merely swapped helpers.
- **6m. A reply into an anonymous root still works.** An operator-composed root with both senders NULL — 105 exist in the live database, 59 `message` and 46 `broadcast` — accepts an actor reply. This is the case that fails if the relaxation is written as `sender_member_id IS NOT NULL` instead of "no other actor owns the root", and no other test distinguishes the two predicates because every other test's root is agent-created.
...
| the actor ack copied from `acknowledge_external_request` with only its own `:339-340` comparison relaxed, keeping the `return await self.request_status(...)` tail | **6j**, and only its status-code assertion. The response path re-enters `_require_actor_owns_thread` through `self.thread`, the route catches `ValueError` alone, and the transition has already committed — measured `HTTP 500` with `request_status == 'acknowledged'`. Every state assertion in 6d and 6k passes against this mutant; what fails is the caller, who sees an error for a write that happened. This is the *default* implementation, which is why the row is here |
| the actor ack moves only `answered → acknowledged` (the external ack's transition, carried over) | **6k**. Measured, the UI's pending-handoff button then returns `200` and changes nothing. Note it is invisible to 6d, whose root is a `context_request` that is already `answered` — a spec requirement stated as "records the acknowledgement" is satisfied by this mutant |
| the frontend swaps `ackAgentMailMessage` for the actor helper but keeps sending `answerToAck.id` | **6l**. The actor ack is root-scoped and refuses a non-request kind, so the answer-ack button becomes a permanent `400 "Message is not a request"` — measured. No backend test sees this; 6l must drive the id the frontend actually sends |
| the reply relaxation written as `root.sender_member_id is not None` ("agent-created" read literally) instead of "no other actor owns the root" | **6m**. Both predicates admit every agent-created root, so all of 6a–6i pass. What breaks is the operator's own anonymous threads — 105 in the live database, 59 of them direct messages — which is the surface this section exists to make usable |

Two rows in this table have an empty test column on purpose — `==` instead of `hmac.compare_digest`, and the operator token exported into the environment rather than read from `backend/.env`. A timing-safe comparison is not test-observable, and neither is *where a settings value was read from*, so listing them as review and deployment items is honest; claiming a test covers either would not be. Note the distinction from test 20's prefix and trailing-byte rows: those catch a comparison that is *wrong*, which is observable; only a comparison that is right-but-not-constant-time is invisible, and that is the residue the review item covers.
...
- `frontend/src/features/agent-mail/api.ts` — three write calls gain actor auth, plus the `sessionStorage` provisioning helper (§3.6). Measured, those three are the whole surface: `sendAgentMailMessage` (called from `AgentMailPage.tsx:174` for compose and `ThreadDialog.tsx:165` for reply) and `ackAgentMailMessage` (`ThreadDialog.tsx:137`). `updateAgentMailMember` (`:163`) is member configuration, not mail. Note two exports that look like they belong and do not: `fetchAgentMailInbox` and `markAgentMailRead` are **dead** — no component calls either — which matters because `agent/inbox` is the route §3.5 makes a write, so a reader who assumes the UI calls it will over-scope this file's change. Two shape changes come with the auth change, and neither is a rename: **compose fans out to four routes**, because the external API has one route per kind (`/messages`, `/broadcasts`, `/context-requests`, `/handoffs`) with `why_needed`/`files_or_symbols`/`files`/`next_steps` as **top-level** fields, whereas `ComposeDialog` nests them under `payload` (`:139-146`) for the single member route; and **`apiClient` discards `response.status`**, throwing `new Error(message)` (`lib/api.ts:117-122`), so test 6h's "`401` re-provisions, `403`/`500` do not" is unimplementable through it unchanged — the actor helpers need the status, which is a deliberate local fetch rather than a global change.
- `backend/app/services/external_agent_mail_service.py` — the §3.6b capability: `reply_in_thread`'s ownership refusal (`:258-259`) becomes conditional on **no other actor** owning the root, its `recipient_member_id` argument becomes `None` (`:265`), and a new actor-scoped acknowledgement lands beside `acknowledge_external_request` (`:330-346`) — writing no member receipt, moving **both** the pending-`handoff` and the answered-`context_request` transitions, and projecting its response through `agent_mail_service.get_thread` rather than `self.request_status`, whose `self.thread` call re-enters the ownership gate. `authenticate_actor` is unchanged and already writes only `actor.last_used_at` (`:117`), which is what makes the ack requirement small. `_require_actor_owns_thread` (`:396-402`) is **not** relaxed: read scope is a separate decision, and the member thread route is already ungated.
- `backend/app/api/v1/external_agent_mail.py` — one new route for the actor ack; the existing 12 are untouched. All are already `Depends(external_actor)`, so no dependency work here. The new route must catch `PermissionError` as well as `ValueError`, unlike the ack route it sits beside (`:228`) — that omission is what turns the gate re-entry into a `500`.
- `frontend/src/features/agent-mail/ThreadDialog.tsx` — the "Reply as" select and its `kind` computation (`:152-156`, `:231-247`, `:263`) are removed, and the answer-ack button sends `root.id` rather than `answerToAck.id` (`:223`). This file is where the design's visible behaviour changes; §3.6b says why the select cannot stay.
