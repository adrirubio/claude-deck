# Agent Mail v2 Implementation Plan
...
3. **State-based, idempotent delivery.** Injections describe current state ("1 unanswered context request"), never events. `SessionStart` hook fires on `startup|resume|clear|compact` sources, so post-compaction recovery is automatic. `read ≠ handled`: receipts track read; request-kind messages carry their own `pending → answered → acknowledged` lifecycle that re-surfaces until resolved.
...
8. **Provider tiers.** Claude Code: one-click install (4 hooks user-scope via `hook_service`, MCP server user-scope via `mcp_service`). Codex CLI: MVP shows copy-paste `config.toml` + `AGENTS.md` snippets (codex_config_service has no MCP mutation seam today; one-click Codex is a fast-follow). Observed-only sessions come free via `discover_agent_sessions()`.
...
| UI kit | shadcn in `src/components/ui/` (badge, button, card, dialog, input, label, select, tabs, textarea, scroll-area available); `CLICKABLE_CARD`, `MODAL_SIZES` from `@/lib/constants`; `MarkdownRenderer` from `@/components/shared/MarkdownRenderer` |
...
```python
    ...
    async def ack_message(self, db: AsyncSession, message_id: int, member_id: int) -> None:
        """Ack a message. Acking an answer (as the original requester) closes the request."""
        result = await db.execute(
            select(MailReceipt).where(
                MailReceipt.message_id == message_id, MailReceipt.member_id == member_id
            )
...
### Task 8: Install service + endpoints (one-click Claude Code, snippets for Codex)
...
"""Install service: hook commands, status detection, apply/uninstall via existing services."""
...
@pytest.mark.asyncio
async def test_apply_adds_missing_hooks_and_mcp():
    with patch.object(install.hook_service, "list_hooks", return_value=[]), \
        ...
        await install.apply_claude_code_install()
    added_events = {call.args[0].event for call in add_hook.call_args_list}
    assert added_events == {"SessionStart", "UserPromptSubmit", "SessionEnd", "PostToolUse"}
    post_tool = next(c.args[0] for c in add_hook.call_args_list if c.args[0].event == "PostToolUse")
...
async def test_uninstall_removes_only_agent_mail_hooks():
    ...
    with patch.object(install.hook_service, "list_hooks", return_value=[ours, theirs]), \
        ...
        await install.uninstall_claude_code()
...
"""Install Agent Mail integration into Claude Code (one-click) and Codex (snippets).

Reuses the existing HookService and MCPService writers so settings.json and
~/.claude.json round-trip exactly like the Hooks and MCP Servers pages.
"""
import shutil
import sys
...
async def install_claude_code():
    return await agent_mail_install_service.apply_claude_code_install()


@router.post("/install/claude-code/uninstall", response_model=AgentMailInstallStatus)
async def uninstall_claude_code():
    return await agent_mail_install_service.uninstall_claude_code()
...
Run: `pytest tests/agent_mail/test_install.py -v`
Expected: 6 passed (the `shim_path` test asserts the path string only; the file itself arrives in Task 9)
...
def deck_ack_message(message_id: int) -> dict:
    ...
    request; acking a handoff signals you have accepted it."""
    err = _guard()
    if err:
        return err
...
export async function applyClaudeCodeInstall(): Promise<AgentMailInstallStatus> {
  return apiClient<AgentMailInstallStatus>(`${BASE}/install/claude-code/apply`, { method: 'POST' })
}

export async function uninstallClaudeCode(): Promise<AgentMailInstallStatus> {
  return apiClient<AgentMailInstallStatus>(`${BASE}/install/claude-code/uninstall`, {
    method: 'POST',
  })
}
...
import {
  applyClaudeCodeInstall, fetchInstallStatus, fetchSnippets, uninstallClaudeCode,
} from './api'
...
export function InstallTab() {
  ...
  const run = async (fn: () => Promise<AgentMailInstallStatus>) => {
    setBusy(true)
    setError(null)
    try {
      setStatus(await fn())
  ...
  return (
    <div className="space-y-4 max-w-3xl">
      <Card>
        <CardHeader>
          <CardTitle className="text-base flex items-center gap-2">
            Claude Code
            {status && (
              <Badge variant={fullyInstalled ? 'default' : 'secondary'}>
                {fullyInstalled ? 'installed' : 'not installed'}
              </Badge>
            )}
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-3 text-sm">
          ...
          <div className="flex gap-2">
            <Button size="sm" disabled={busy || fullyInstalled} onClick={() => run(applyClaudeCodeInstall)}>
              Install
            </Button>
            <Button size="sm" variant="outline" disabled={busy} onClick={() => run(uninstallClaudeCode)}>
              Uninstall
            </Button>
          </div>
        </CardContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Codex CLI (manual)</CardTitle>
        </CardHeader>
        <CardContent className="space-y-3 text-sm">
          <p className="text-muted-foreground">
            Codex has no lifecycle hooks; it participates via MCP and an AGENTS.md standing
            instruction. Add this to ~/.codex/config.toml:
          </p>
          ...
          <p className="text-muted-foreground">And this to ~/.codex/AGENTS.md:</p>
...
## Install            (Claude Code one-click; Codex manual TOML + AGENTS.md; restart note)
...
## Security & limits  (local trust model — no auth token, same posture as the rest of the
                      ...
                      sharing; one member per repo in MVP; leases/PreToolUse guard are
                      future work)
...
```
Expected: all backend tests pass (existing + ~35 new), frontend build + lint clean.
...
1. **HTTP-response-as-hook-output**: the design assumes a `curl` command hook's stdout (= endpoint response JSON) is parsed for `hookSpecificOutput.additionalContext`. This is the documented command-hook contract; smoke test 4 verifies it early — if it fails, the fallback is wrapping in `jq -r` extraction or a tiny wrapper script. **Verify in smoke 4 before building further UI polish.**
...
Path leases + PreToolUse guard (for shared-working-tree multi-agent), one-click Codex `config.toml` mutation, WebSocket realtime UI (reuse presence WS pattern), multi-member-per-repo seats, scopes/workspaces if the flat roster gets noisy, optional API token hardening.
